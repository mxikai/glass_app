...
class InventoryView(QWidget):
    ...
    def setup_ui(self):
        ...
        subtitle = QLabel("Inventory items are physical assets connected to expense transactions.")
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Expense Txn", "Item Name", "Qty", "Condition", "Status", "Date Recorded"
        ])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        # Make the ID and Qty columns a bit smaller
        ...
        # Apply CSS styling
        for inp in [self.input_id, self.input_txn, self.input_name, self.input_qty, 
                    self.input_condition, self.input_status, self.input_date]:
            inp.setObjectName("formInput")

        form_layout.addRow("Inventory ID", self.input_id)
        form_layout.addRow("Expense Txn *", self.input_txn)
    ...
    def load_all_data(self):
        try:
            ...
            # Inventory items can ONLY be tied to Expenses per your documentation
    ...
    def refresh_table(self):
        ...
        for row_idx, item in enumerate(self.inventory_data):
            ...
            self.table.setItem(row_idx, 1, QTableWidgetItem(f"Txn #{item.get('transaction_id', '')}"))
            ...
            self.table.setItem(row_idx, 4, QTableWidgetItem(str(item.get("item_condition", ""))))
            self.table.setItem(row_idx, 5, QTableWidgetItem(str(item.get("status", "Active"))))
            
            raw_date = str(item.get("date_recorded", ""))
            clean_date = raw_date[:10] if raw_date else "Unknown"
            self.table.setItem(row_idx, 6, QTableWidgetItem(clean_date))
